...
func (l *auth) Authenticate(ctx context.Context, requestData *user.LoginRequest) (*user.LoginResponse, error) {
...
func (l *auth) ForgotPassword(ctx context.Context, email string) error {
	...
	go func() {
		...
		if err := l.smtp.
			...
			SetHTMLTemplate(l.templateFolder, common.EmailTemplate[common.ForgotPassword], "Request To Change Password", args).
			Send(); err != nil {
...
func (l *auth) ResendToken(ctx context.Context, email string) error {
	cacheKey := fmt.Sprintf("%s:otp-request:%s", common.PrefixCacheKey, email)
...
func (l *auth) UpdatePassword(ctx context.Context, requestData *user.ChangePasswordRequest) error {
	...
	if _, err := l.repo.Authenticate(ctx, &user.LoginRequest{
		Email:    jwtData.Email,
